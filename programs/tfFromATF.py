...
def director(cv):
    ...
    def processMeta():
        ...
        if match:
            ...
            trans = match.group(2)
            if not curLine:
                errors[f"meta: translation outside line"][src].add(
        ...
        if fields[0] == "atf:":
            ...
            if value.startswith("="):
                ...
                errors[f"meta: spurious ="][src].add(
    ...
    def processAtSpec():
        ...
        if typ == "column" or typ in COL_CORRECTION:
            if typ in COL_CORRECTION:
                ...
                errors[f"structure: column correction"][src].add(
            ...
            if typ in FACES_CORRECTION:
                ...
                errors[f"structure: face correction"][src].add(
    ...
    def commentInsert(meta=False):
        ...
        if meta:
            if transLine is None:
                errors[f"comment: # line without preceding transcription line"][
                    src
                ].add((i, line, pNum, comment))
            else:
    ...
    def lineStart(ln):
        ...
        cv.feature(
            curLine, ln=ln, srcfile=src, srcLnNum=i, srcLn=line,
        )
        if hasPrimeLn:
        ...
        if changed:
            errors[f"cluster: space near edge"][src].add(
            ...
            while word:
                ...
                if c == "-" or c in operatorSet:
                    ...
                    if inSign and not endPart:
                        ...
                        errors[
                            f"sign: flag attached to cluster (applied to sign instead)"
            ...
            for line in fh:
                ...
                if isDoc:
                    if len(line) > 1 and line[1] == "P":
                        ...
                        errors[f"atf: stray & replaced by $"][src].add(
                            (i, line, pNum, None)
                        )
                        commentInsert()
                ...
                if not isMeta or not line.startswith("#tr."):
                    ...
                    if msg:
                        errors[f"atf: illegal character(s)"][src].add(
                            (i, lineMsg, pNum, msg)
                        )
